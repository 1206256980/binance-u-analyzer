...
public class KlineFetcher {
    ...
    private final ConcurrentHashMap<String, long[]> timesCache = new ConcurrentHashMap<>();
    ...
    public double[] fetchCloses(String symbol) throws IOException {
        ...
        long[] times = new long[n];        // <----- 新增

        for (int i = 0; i < n; i++) {
            JsonArray item = arr.get(i).getAsJsonArray();
            times[i] = item.get(0).getAsLong();   // openTime
            lows[i] = item.get(3).getAsDouble();
            closes[i] = item.get(4).getAsDouble();
        }

        closesCache.put(symbol, closes);
        lowsCache.put(symbol, lows);
        timesCache.put(symbol, times);      // <----- 新增
    ...
    public long[] fetchTimes(String symbol) throws IOException {
        if (timesCache.containsKey(symbol)) return timesCache.get(symbol);
        fetchCloses(symbol);
        return timesCache.get(symbol);
    }
}
